...
const readFileAsync = async (path) => {
    return new Promise((resolve, reject) => {
        node_fs_1.readFile(path, "utf8", (err, data) => {
            if (err || !data) {
...
const writeFileAsync = async (path, data) => {
    return new Promise((resolve, reject) => {
        node_fs_1.writeFile(path, data, (err) => {
            if (err) {
...
const modifyPodfile = (config, props) => {
    config = config_plugins_1.withXcodeProject(config, async (_props) => {
